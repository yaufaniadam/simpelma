...
class Prodi extends MY_Controller 
{
	...
	public function edit($id_prodi)
	{
		if ($this->input->post('submit')) {
			...
			$this->form_validation->set_rules('admin_prodi[]', 'Admin Prodi', 'required',
					array('required' => '%s wajib diisi.')
			);
			$this->form_validation->set_rules('ka_prodi[]', 'Kepala Prodi', 'required',
					array('required' => '%s wajib diisi.')
			);
			...
			if ($this->form_validation->run() == FALSE) {
                ...
                $data['admin_prodi'] = $this->prodi_model->get_admin_prodi($id_prodi, 2);
				$data['ka_prodi'] = $this->prodi_model->get_admin_prodi($id_prodi, 6);
